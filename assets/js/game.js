...
function renderImages(images) {
  ...
  const isKeyboard = images.length > 0 && images[0].image.includes('Gaming Tastatur');
...
function createProduct() {
  ...
  if (!name || !desc || !meta || !alt || !price || !selectedImageUrl) {
    if (feedback) {
      feedback.innerText = !selectedImageUrl ? 'Bitte wähle zuerst das richtige Produktbild aus!' : 'Bitte fülle alle Felder aus!';
  ...
  if (vat !== correctVat) {
    const errorText = (currentKey === 'protein') 
  ...
  if (feedback) {
    feedback.innerText = 'Produkt erfolgreich erstellt! Klicke auf Speichern, um fortzufahren.';
    feedback.style.display = 'block';
...
function renderKeywordsForCurrentProduct() {
  ...
  if (key && products[key] && puzzle) {
    const product = products[key];
    const allKeywords = [...product.correctKeywords, ...product.wrongKeywords].sort(() => Math.random() - 0.5);
    puzzle.innerHTML = allKeywords.map(kw => `
...
function checkSEOPuzzle() {
  ...
  const product = products[key];
  const selectedWrongWords = seoWords.filter(w => product.wrongKeywords.includes(w));
  const selectedCorrectWords = seoWords.filter(w => product.correctKeywords.includes(w));
...
function checkConversion() {
  ...
  const convincingWords = ['garantie', 'kostenlos', 'exklusive', 'sichern', 'jetzt', 'sofort', 'limitiert', 'angebot', 'versand'];
  const hasConvincingWord = convincingWords.some(word => text.toLowerCase().includes(word));
...
function updateShopStyle(type, value, element) {
  ...
  if (element) {
    element.parentElement.querySelectorAll('.secondary-btn, .color-swatch').forEach(el => el.classList.remove('active-choice'));
  ...
  if (type === 'theme') {
    ...
    const variantButtons = document.querySelectorAll('#previewMain [style*="border: 2px solid"]');
    ...
    const activeThumbnail = document.querySelector('#previewImageWrapper [style*="border: 2px solid #38bdf8"]');
  ...
  if (type === 'headerFooterColor') {
    ...
    if (value === 'glass') {
      ...
      if (footer) {
        ...
        footer.querySelectorAll('span, div').forEach(el => el.style.color = '#1e293b');
      }
    } else {
  ...
  if (type === 'newsletterBg' && preview) {
    ...
    if (newsletter) {
      ...
      const darkColors = [
        '#0f172a', '#1e293b', '#000000', '#6366f1', '#a855f7', '#8b5cf6', '#ef4444', '#f43f5e'
      ];
      ...
      if (nlInputGroup) {
        nlInputGroup.style.setProperty('border', `1px solid ${borderColor}`, 'important');
      }
    }
  }
  if (type === 'nav' && nav) {
...
function toggleShopElement(elementId, element) {
  ...
  if (el) {
    ...
    el.style.display = isHidden ? (elementId === 'trust-badges' ? 'flex' : 'block') : 'none';
    if (element) isHidden ? element.classList.add('active-choice') : element.classList.remove('active-choice');
  }
}
...
function loadShopBuilderData() {
  ...
  const desc = document.getElementById('productDescription')?.value || 'Beschreibung...';
  ...
  if (pPrice) {
    const formattedPrice = parseFloat(price.toString().replace(',', '.')).toFixed(2).replace('.', ',');
...
function finalizeShopDesign() {
  ...
  showModal('Shop veröffentlicht!', 'Du hast alle Aufgaben gemeistert!\nGlückwunsch, dein Shop ist jetzt live.', 'Beenden');
}
